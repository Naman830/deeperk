...
- [x] **Chat verified end-to-end — and the harness is now permanent** (2026-08-18). `tests/` is a checked-in Vitest suite (15 files, 187 tests: every REST route, every socket event, the internal event bridge, pages) run with `npm test` against `npm run dev` + the real Neon DB; see `tests/README.md`. It found and pinned one real server bug (sends emitted right after `session:ready` were silently dropped) and the unread-divider misplacement.
- [x] **Production-readiness QA pass** (2026-08-20). Full-app sweep: voice-note hardening (server-side 2-min duration bound, upload-failure retry keeps the take, tombstone completeness, resource-type map for the future media sweep), a connect-time `presence:snapshot` (stale "offline" no longer sticks), network-failure fetches no longer fail silently, dead-code removal, and a **permanent Playwright browser suite** (`tests/browser/`, `npm run test:browser` — signup/login/chat/voice/calls/search/settings/mobile with a fake mic and real WebRTC). See CLAUDE.md → "Production-readiness QA pass".

### Deploy-day checklist

Pure environment configuration — nothing to code, nothing meaningful until something is deployed. [`Docs/deployment/deploy.md`](./deployment/deploy.md) carries the exact copy-paste instructions for each:
